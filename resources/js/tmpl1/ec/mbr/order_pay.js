...
$(function () {
    ...
    $("#sub-orders").click(function () {//提交订单
        ...
        $.postForm(vars.root + "/ec/mbr/confirm_pay.html", data_confirm);
    });
...
$(function () {
    ...
    $("#sub-orders").click(function () {//提交订单
        loader();
        var confirmPayUrl=vars.root + "/ec/mbr/doConfirmEcOrderPay.do";
        var orderNoStr = $("#orderNoStr").val();
        var useBalance = $("#useBalance").val();
        var useCashCoupons = $("#useCashCoupons").val();
        var useCredits = $("#useCredits").val();
                ...
                success:function(data){
                    ...
                    if (data.success) {
                        if(data.data.orders.length>0 && data.data.payId){
                            ...
                            var url='';
                            //获取调起支付参数
                            var url2=vars.root+"/wx/pay/doUnifiedorder.html";
                            var postData2={
                                payId:payId
                            };
                ...
                if (data.success) {
                    if(data.data.orders.length>0 && data.data.payId){
                        ...
                        if (payTypeId == 3) {//微信支付
                            ...
                            url= vars.root+"/ec/mbr/user_order.html";
                        }
                        location.href=url;
                    }else{
                        mui.alert('提交失败');
